  ...
  template: `
    <div class="flex flex-col gap-2 items-center text-center">
      @switch (config.data?.icon) {
        @case (Icon.Question) {
          <img src="images/question-image.png" alt="question" class="w-14 h-14" />
        }
        @case (Icon.Info) {
          <img src="images/question-image.png" alt="info" class="w-14 h-14" />
        }
        @case (Icon.Warning) {
          <img src="images/warning-image.png" alt="warning" class="w-14 h-14" />
        }
        @default {
          <img src="images/warning-image.png" alt="warning" class="w-14 h-14" />
        }
      ...
      @if (messageText) {
        <p class="text-sm text-surface-sub whitespace-nowrap">{{ messageText }}</p>
      }
      <div class="flex w-full gap-3 mt-1">
        <p-button class="flex-1" [fluid]="true" [severity]="'secondary'" [outlined]="true" (onClick)="handleCancel()">
          {{ config.data?.cancelButtonLabel ?? "ยกเลิก" }}
        </p-button>
        <p-button class="flex-1" [fluid]="true" severity="primary" [loading]="confirmLoading" (onClick)="handleConfirm()">
