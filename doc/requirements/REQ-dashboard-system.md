...
| กราฟ | ไม่มี | Line (trend), Bar (top selling, peak hours), Pie (category) |
...
- ออเดอร์แยกครัว (อาหาร / เครื่องดื่ม / ของหวาน) — จำนวนรายการ SUM(Quantity) แต่ละหมวด
- Revenue Trend (line chart 7/30 วัน)
- เมนูขายดี Top 5 แยก 3 หมวด (อาหาร/เครื่องดื่ม/ของหวาน)
- ช่วงเวลาขายดี (hourly bar chart)
- Sales Report (period filter + KPI summary + pie chart หมวดหมู่ + ออเดอร์แยกครัว + daily table)
...
│                                                                    │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐          │
│  │ ยอดขาย   │  │ ออเดอร์  │  │ ลูกค้า   │  │ เฉลี่ย/ออเดอร์│          │
│  │ ฿12,450  │  │ 28      │  │ 45       │  │ ฿444.64  │          │
│  │ ▲ +15.2% │  │ ▲ +7.7% │  │ ▼ -3.2%  │  │ ▲ +6.9%  │          │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘          │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────┐      │
│  │  ออเดอร์แยกครัว (วันที่เลือก)                              │      │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐               │      │
│  │  │ 🍳 อาหาร │  │ 🥤 เครื่อง│  │ 🍰 ของหวาน│               │      │
│  │  │  45 ชิ้น │  │  22 ชิ้น │  │  12 ชิ้น │               │      │
│  │  └──────────┘  └──────────┘  └──────────┘               │      │
│  └──────────────────────────────────────────────────────────┘      │
│                                                                    │
│  ┌────────────────────────────────────────────────────────────┐    │
...
- **Row 2**: ออเดอร์แยกครัว — 3 mini cards (อาหาร / เครื่องดื่ม / ของหวาน) แสดง SUM(Quantity) ตาม CategoryType
- **Row 3**: Revenue Trend Line Chart (full width) พร้อม toggle 7/30 วัน
- **Row 4**: Top Selling 3 Horizontal Bar Charts (grid 3 columns → 1 column บน mobile)
- **Row 5**: Peak Hours Vertical Bar Chart (full width)
...
| จำนวนออเดอร์ | `pi pi-shopping-cart` | TbOrder | `COUNT(*) WHERE DATE(CreatedAt)=selectedDate` (Global Query Filter ซ่อน deleted records อัตโนมัติ) |
| จำนวนลูกค้า | `pi pi-users` | TbOrder | `SUM(GuestCount) WHERE DATE(CreatedAt)=selectedDate` |
| ยอดเฉลี่ยต่อออเดอร์ | `pi pi-chart-bar` | คำนวณ | ยอดขาย / จำนวนออเดอร์ (ถ้า 0 → แสดง "฿0") |

### 3.2 ออเดอร์แยกครัว

| หมวด | CategoryType | Data Source | สูตร |
|------|:---:|-----------|------|
| อาหาร | 1 | TbOrderItem | `SUM(Quantity) WHERE CategoryType=1 AND Status NOT IN (VOIDED, CANCELLED) AND DATE(CreatedAt)=selectedDate` |
| เครื่องดื่ม | 2 | TbOrderItem | เหมือนข้างบน CategoryType=2 |
| ของหวาน | 3 | TbOrderItem | เหมือนข้างบน CategoryType=3 |

- แสดงเป็น 3 mini cards เรียงแนวนอน (grid 3 columns)
- แต่ละ card แสดง: icon ครัว + ชื่อหมวด + จำนวนรายการ (ชิ้น)
- **ไม่มีเปรียบเทียบ %** (เพื่อไม่ให้ข้อมูลรกเกินไป)

### 3.3 เปรียบเทียบวันก่อนหน้า
...
│                                                                    │
│  ┌────────────────────────┐  ┌────────────────────────┐           │
│  │ สัดส่วนยอดขาย          │  │ ออเดอร์แยกครัว          │           │
│  │ ตามหมวดหมู่             │  │                        │           │
│  │     ╭────╮             │  │ อาหาร       150 (58%)  │           │
│  │   ╱ อาหาร ╲            │  │ เครื่องดื่ม    80 (31%)  │           │
│  │  │  60%    │           │  │ ของหวาน      28 (11%)  │           │
│  │   ╲ เครื่อง ╱           │  │                        │           │
│  │     ╰────╯             │  │                        │           │
│  │  ของหวาน 15%           │  │                        │           │
...
│  │  ยอดขายรายวัน                                               │    │
│  │  ┌────────┬──────────┬────────┬────────┬──────────┐        │    │
│  │  │ วันที่  │ ยอดขาย   │ ออเดอร์ │ ลูกค้า │ เฉลี่ย/ออเดอร์│        │    │
│  │  ├────────┼──────────┼────────┼────────┼──────────┤        │    │
...
- **Data**: `SUM(Quantity) FROM TbOrderItem WHERE Status NOT IN (VOIDED, CANCELLED) GROUP BY CategoryType`
- **3 items**: อาหาร (primary) / เครื่องดื่ม (success) / ของหวาน (warning)
- แสดงจำนวนรายการ (ชิ้น) + % ของทั้งหมด
- แสดงเป็นรายการแนวตั้ง (ไม่ใช่ Pie chart) — แถบสี + ชื่อหมวด + จำนวน + %
...
| เฉลี่ยต่อออเดอร์ | `w-[140px]` | format สกุลเงิน |
...
| GET | `/api/dashboard/sales-report` | `from`, `to` | Summary + daily breakdown + category breakdown + kitchen breakdown |
...
```json
{
  ...
  "result": {
    "selected": {
      ...
      "orderCount": 28,
      "guestCount": 45,
      "averagePerOrder": 444.64
    },
    "previous": {
      "totalSales": 10800.00,
      "orderCount": 26,
      "guestCount": 46,
      "averagePerOrder": 415.38
    },
    "kitchenBreakdown": [
      { "categoryType": 1, "categoryName": "อาหาร", "itemCount": 45 },
      { "categoryType": 2, "categoryName": "เครื่องดื่ม", "itemCount": 22 },
      { "categoryType": 3, "categoryName": "ของหวาน", "itemCount": 12 }
    ],
    "revenueTrend": [
...
```json
{
  ...
  "result": {
    ...
    "kitchenBreakdown": [
      { "categoryType": 1, "categoryName": "อาหาร", "itemCount": 150, "percentage": 58.0 },
      { "categoryType": 2, "categoryName": "เครื่องดื่ม", "itemCount": 80, "percentage": 31.0 },
      { "categoryType": 3, "categoryName": "ของหวาน", "itemCount": 28, "percentage": 11.0 }
    ]
  }
}
```
...
| ระบบ | ความสัมพันธ์ |
|------|-------------|
| **Order System** ([REQ-order-system](REQ-order-system.md)) | TbOrder (count, guest count, CreatedAt), TbOrderItem (quantity, CategoryType, status filter, MenuNameThai, OrderId) — aggregate queries |
...
3. เห็นออเดอร์แยกครัว (อาหาร / เครื่องดื่ม / ของหวาน) → รู้ว่าครัวไหนมีงานเยอะ
4. เห็น Revenue Trend 7 วัน → toggle ดู 30 วัน
5. เห็น Top Selling 3 หมวด → รู้ว่าเมนูไหนขายดี
6. เห็น Peak Hours → รู้ช่วงเวลาที่ลูกค้าเยอะ
...
2. กดปุ่ม "เดือนนี้"
3. เห็น KPI Summary, Pie chart (หมวดหมู่), ออเดอร์แยกครัว, ตารางรายวัน
...
| [REQ-self-order-system](REQ-self-order-system.md) | TbCustomerSession — ลูกค้าสั่งเอง (อ้างอิงทั่วไป) |
