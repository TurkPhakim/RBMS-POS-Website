...
@Component({
  ...
  template: `
    <div class="bg-surface-card rounded-xl border" [ngClass]="borderClass">
      <!-- Gradient Header -->
      <div
        class="bg-gradient-to-r px-5 py-4 rounded-t-xl relative overflow-hidden"
        [ngClass]="gradientClass"
      >
        <div
          class="absolute -top-6 -right-6 w-24 h-24 rounded-full bg-white/10"
        ></div>
        <div
          class="absolute -bottom-4 -left-4 w-20 h-20 rounded-full bg-white/10"
        ></div>
