...
ในบทนี้จะกล่าวถึงการวิเคราะห์และออกแบบระบบของระบบบริหารธุรกิจร้านอาหารผ่านเว็บพร้อมระบบ POS เพื่อให้ตอบสนองต่อความต้องการของผู้ใช้งานและวัตถุประสงค์ของโครงงาน ซึ่งมีรายละเอียดดังนี้ สถาปัตยกรรมระบบแบบหลายชั้น (N-Tier Architecture) ความต้องการของระบบทั้งด้านฟังก์ชันการทำงาน (Functional Requirements) และที่อยู่นอกเหนือฟังก์ชันการทำงาน (Non-Functional Requirements) แผนภาพกรณีการใช้งาน (Use Case Diagram) ของผู้ใช้งาน 3 กลุ่ม ภาพรวมการทำงานของระบบ (System Overview Diagram) แผนภาพการทำงานของระบบ (System Flowchart)การออกแบบฐานข้อมูลในรูปแบบพจนานุกรมข้อมูล (Data Dictionary) สุดท้ายคือแผนผังความสัมพันธ์ของเอนทิตี (Entity-Relationship Diagram)
...
```mermaid
flowchart TD
    Start([เริ่มต้น]) --> A[สถานะเริ่มต้น: Available]
    A --> B{เหตุการณ์?}
    B -- ลูกค้าจอง --> C[เปลี่ยนเป็น Reserved]
    B -- ลูกค้านั่งทันที --> D[เปลี่ยนเป็น Occupied]
    B -- ปิดใช้งานชั่วคราว --> E[เปลี่ยนเป็น Unavailable]
    C --> F{เหตุการณ์ในวันจอง?}
    F -- ลูกค้ามาตามจอง --> D
    F -- ยกเลิกการจอง --> M[กลับเป็น Available]
    D --> G[ปิดบิล<br/>เปลี่ยนเป็น Billing]
    G --> H[ชำระเงินเสร็จ<br/>เปลี่ยนเป็น Cleaning]
    H --> I[ทำความสะอาดเสร็จ<br/>กลับเป็น Available]
    E --> J[เปิดใช้งานอีกครั้ง<br/>กลับเป็น Available]
    I --> End([สิ้นสุด])
    J --> End
    M --> End
```
...
- สถานะเริ่มต้นของโต๊ะคือ Available ซึ่งสามารถเปลี่ยนเป็น Reserved เมื่อมีการจองล่วงหน้า หรือเปลี่ยนเป็น Occupied เมื่อลูกค้านั่งทันที
- เมื่อโต๊ะอยู่ในสถานะ Reserved ระบบรอเหตุการณ์ในวันจอง หากลูกค้ามาตามจองจะเปลี่ยนเป็น Occupied หากยกเลิกจะกลับเป็น Available
...
- สถานะ Unavailable ใช้สำหรับโต๊ะที่ปิดใช้งานชั่วคราว เช่น โต๊ะที่ชำรุดหรืออยู่ในระหว่างซ่อมแซม และสามารถเปิดใช้งานกลับเป็น Available ได้เมื่อพร้อม
...
flowchart TD
    ...
    A -- เรียกพนักงาน --> B[Notification ไปกลุ่ม Floor]
    A -- ขอเช็คบิล --> C[Order/Table → Billing<br/>+ สร้าง OrderBill<br/>+ Notification ไปกลุ่ม Cashier]
    A -- ขอชำระเงินสด --> D[Notification ไปกลุ่ม Floor + Cashier]
    A -- ขอแยกบิล --> E[Notification ไปกลุ่ม Floor]
...
**คำอธิบายขั้นตอนการทำงาน:**
- ลูกค้าที่สั่งอาหารผ่าน Self-Order สามารถโต้ตอบกับพนักงานได้ผ่านปุ่มในหน้า Mobile Web ทั้งหมด 4 ปุ่ม ได้แก่ เรียกพนักงาน (Call Waiter), ขอเช็คบิล (Request Bill), ขอชำระเงินสด (Request Cash) และ ขอแยกบิล (Request Split Bill)
- การกดแต่ละปุ่มจะสร้าง Notification ผ่าน SignalR ไปยังกลุ่มพนักงานที่เกี่ยวข้อง โดยปุ่มขอเช็คบิลจะเปลี่ยนสถานะของออเดอร์เป็น Billing และสร้าง OrderBill อัตโนมัติ
...
### 3.5.18 แผนภาพการทำงานส่วน Kitchen Display และการเปลี่ยนสถานะรายการอาหาร

```mermaid
flowchart TD
    Start([เริ่มต้น]) --> A[ลูกค้า/พนักงานเพิ่มรายการอาหาร<br/>OrderItem Status: Pending]
    A --> B{เหตุการณ์ก่อนส่งครัว?}
    B -- กดส่งครัว --> C[Status: Sent<br/>+ SignalR Event ไป Kitchen Group]
    B -- ยกเลิกก่อนส่งครัว --> D[Status: Voided<br/>ไม่ต้องระบุเหตุผล]
    C --> E{การดำเนินการของครัว?}
    E -- ครัวกดเริ่มทำ --> F[Status: Preparing<br/>+ SignalR Event ไปหน้าร้าน]
    E -- ยกเลิกพร้อมระบุเหตุผล --> G[Status: Cancelled<br/>+ บันทึกเหตุผลและผู้ยกเลิก]
    F --> H{เหตุการณ์ถัดไป?}
    H -- ทำอาหารเสร็จ --> I[Status: Ready<br/>+ SignalR Event ไปหน้าร้าน]
    H -- ยกเลิกพร้อมระบุเหตุผล --> G
    I --> J[พนักงานยกอาหารไปเสริฟลูกค้า]
    J --> K[Status: Served]
    D --> End([สิ้นสุด])
    G --> End
    K --> End
...
**คำอธิบายขั้นตอนการทำงาน:**
- รายการอาหารแต่ละรายการมีสถานะทั้งหมด 7 สถานะ ตาม EOrderItemStatus ได้แก่ Pending, Sent, Preparing, Ready, Served, Voided และ Cancelled
- เมื่อลูกค้าหรือพนักงานเพิ่มรายการอาหาร ระบบจะกำหนดสถานะเริ่มต้นเป็น Pending จนกว่าจะกดส่งครัว ซึ่งจะเปลี่ยนเป็น Sent พร้อมส่ง SignalR Event ไปยัง Kitchen Group ทันที
- ครัวกดเริ่มทำ (Preparing) และเมื่อทำเสร็จ (Ready) ระบบจะส่ง SignalR Event ไปยังหน้าร้านเพื่อแจ้งพนักงานเสริฟให้ทราบทั้งสองขั้นตอน
- การยกเลิกแบ่งออกเป็น 2 รูปแบบ ได้แก่ Voided (ยกเลิกก่อนส่งครัว — ไม่ต้องระบุเหตุผล) และ Cancelled (ยกเลิกหลังส่งครัวหรือระหว่างทำ — ต้องระบุเหตุผลและผู้ยกเลิก)
- เมื่อรายการอยู่ในสถานะ Ready แล้ว ระบบจะไม่อนุญาตให้ยกเลิกได้อีก เนื่องจากอาหารถูกปรุงเสร็จเรียบร้อยแล้ว และจะเปลี่ยนเป็น Served เมื่อพนักงานยกอาหารไปเสริฟลูกค้า

### 3.5.19 แผนภาพการทำงานส่วนวงจรชีวิตของออเดอร์ (Order Lifecycle)

```mermaid
flowchart TD
    Start([เริ่มต้น]) --> A[เปิดโต๊ะ + สร้าง Order<br/>Status: Open]
    A --> B{เหตุการณ์ใน Order?}
    B -- เพิ่ม/แก้ไขรายการอาหาร --> B
    B -- ลูกค้า/พนักงานกดปิดบิล --> C[Status: Billing<br/>+ สร้าง OrderBill 1 ใบ<br/>หรือหลายใบ กรณี Split Bill]
    C --> D{เหตุการณ์ต่อไป?}
    D -- ยกเลิกการปิดบิล/สั่งเพิ่ม --> B
    D -- ทุกบิลชำระเงินสำเร็จ --> E[Status: Completed<br/>+ Table Status: Cleaning]
    E --> End([สิ้นสุด])
```

**คำอธิบายขั้นตอนการทำงาน:**
- สถานะของออเดอร์ในระบบมีทั้งหมด 3 สถานะ ตาม EOrderStatus ได้แก่ Open, Billing และ Completed
- เมื่อพนักงานเปิดโต๊ะ ระบบจะสร้าง Order ใหม่ด้วยสถานะเริ่มต้นเป็น Open ในระหว่างนี้ลูกค้าหรือพนักงานสามารถเพิ่มหรือแก้ไขรายการอาหารได้อย่างต่อเนื่อง
- เมื่อกดปิดบิล ระบบจะเปลี่ยนสถานะเป็น Billing พร้อมสร้าง OrderBill หนึ่งใบหรือหลายใบ (กรณี Split Bill) และในสถานะนี้ยังสามารถย้อนกลับไป Open ได้หากลูกค้าต้องการสั่งอาหารเพิ่มเติมหรือต้องการยกเลิกการปิดบิล
- เมื่อทุกบิลภายในออเดอร์ชำระเงินสำเร็จ ระบบจะเปลี่ยนสถานะของ Order เป็น Completed และเปลี่ยนสถานะของโต๊ะเป็น Cleaning เพื่อเตรียมรับลูกค้ารายต่อไป
