...
import { ReceiptDataModel } from '@app/core/api/models/receipt-data-model';
import { ShopBrandingService } from './shop-branding.service';
...
export class ReceiptService {
  ...
  constructor(
    ...
    private shopBranding: ShopBrandingService,
    private http: HttpClient,
  ) {}

  downloadReceipt(paymentId: number): Observable<void> {
    return this.ensureLogo().pipe(
      switchMap(() =>
        this.paymentsService.paymentsGetReceiptDataGet({ paymentId }),
      ),
      map((res) => {
        if (res.result) {
  ...
  downloadConsolidatedReceipt(orderId: number): Observable<void> {
    return this.ensureLogo().pipe(
      switchMap(() =>
        this.paymentsService.paymentsGetConsolidatedReceiptGet({ orderId }),
      ),
      map((res) => {
        if (res.result) {
          this.generatePdf(res.result);
  ...
  private ensureLogo(): Observable<void> {
    ...
    const url = this.shopBranding.logoUrl();
    if (this.logoBase64 && this.cachedLogoUrl === url) {
      return of(undefined);
    }
