...
export const layoutReducer = createReducer(
  ...
  // --- Notification Panel ---
  on(LayoutActions.toggleNotificationPanel, (state) => ({
    ...state, notificationPanelOpen: !state.notificationPanelOpen
  })),
  on(LayoutActions.closeNotificationPanel, (state) => ({
  ...
  on(LayoutActions.addOrUpdateHeaderButton, (state, { button }) => {
    ...
    const headerButtons = exists
      ? state.headerButtons.map(b => b.key === button.key ? { ...b, ...button } : b)
      : [...state.headerButtons, button];
    return { ...state, headerButtons };
  }),
  on(LayoutActions.removeHeaderButton, (state, { key }) => ({
    ...state, headerButtons: state.headerButtons.filter(b => b.key !== key)
  })),
  on(LayoutActions.setHeaderButtonLoading, (state, { key, loading }) => ({
    ...state, headerButtons: updateButtonInArray(state.headerButtons, key, btn => ({ ...btn, loading }))
  })),
  on(LayoutActions.setHeaderButtonDisabled, (state, { key, disabled }) => ({
    ...state, headerButtons: updateButtonInArray(state.headerButtons, key, btn => ({ ...btn, disabled }))
  })),
  ...
  on(LayoutActions.addOrUpdateBreadcrumbButton, (state, { button }) => {
    ...
    const breadcrumbButtons = exists
      ? state.breadcrumbButtons.map(b => b.key === button.key ? { ...b, ...button } : b)
      : [...state.breadcrumbButtons, button];
    return { ...state, breadcrumbButtons };
  }),
  on(LayoutActions.removeBreadcrumbButton, (state, { key }) => ({
    ...state, breadcrumbButtons: state.breadcrumbButtons.filter(b => b.key !== key)
  })),
  on(LayoutActions.setBreadcrumbButtonLoading, (state, { key, loading }) => ({
    ...state, breadcrumbButtons: updateButtonInArray(state.breadcrumbButtons, key, btn => ({
      ...btn, item: { ...btn.item, loading }
    }))
  })),
  on(LayoutActions.setBreadcrumbButtonDisabled, (state, { key, disabled }) => ({
    ...state, breadcrumbButtons: updateButtonInArray(state.breadcrumbButtons, key, btn => ({
      ...btn, item: { ...btn.item, disabled }
    }))
  })),
