...
ตาราง Join สำหรับเชื่อมโต๊ะหลายตัวเข้าด้วยกัน (เช่น รวม A1+A2) — เมื่อ Link จะ **รวม Order เป็นตัวเดียว** (Primary table เป็นเจ้าของ), Unlink จะ **แยก items กลับตาม SourceTableId** — **Hard delete**

| คอลัมน์                       | ชนิด         | จำเป็น | รายละเอียด                            |
| ----------------------------- | ------------ | ------ | ------------------------------------- |
| `TableLinkId`                 | int          | ✅ PK  | Identity auto-increment               |
| `GroupCode`                   | nvarchar(50) | ✅     | รหัสกลุ่ม (โต๊ะที่ GroupCode เดียวกันเชื่อมกัน) |
| `IsPrimary`                   | bit          | ✅     | โต๊ะหลัก (เจ้าของ Order — เมื่อ link จะรวม Order ไปที่โต๊ะนี้) Default: false |
...
| TbOrderBills → TbServiceCharges               | M:1 (Optional) | `TbOrderBills.ServiceChargeId`            | SetNull         | บิลผูกกับค่าบริการ (nullable — ถ้าลบ SC จะ set null)     |
| TbOrderBills → TbOrderItems                   | 1:M (Optional) | `TbOrderItems.OrderBillId`                | SetNull         | บิล 1 ใบมีได้หลายรายการ (ByItem split — ถ้าลบบิล จะ set null) |
| TbOrderItems → TbTables (SourceTable)         | M:1 (Optional) | `TbOrderItems.SourceTableId`              | Restrict        | โต๊ะต้นทางที่สั่ง item (ใช้ตอน link/unlink โต๊ะ — แยก items กลับ) |
...
| GET    | `/order/{orderId}/consolidated-receipt` | ใบเสร็จรวมทุกบิลของ Order          | `payment-manage.read`   | URL param: orderId                            | `BaseResponseModel<ReceiptDataModel>`           |
...
| GET    | `/consolidated-receipt`                 | ใบเสร็จรวมทุกบิลของ Order (ลูกค้า)         | CustomerAuthorize | -                                      | `BaseResponseModel<ReceiptDataModel>`                |
...
Route prefix: `api/dashboard` — **PermissionAuthorize**: `dashboard.view.read`

| Method | Endpoint        | วัตถุประสงค์                              | Permission             | Request                     | Response                                                  |
| ------ | --------------- | ----------------------------------------- | ---------------------- | --------------------------- | --------------------------------------------------------- |
| GET    | `/overview`     | ภาพรวมวันที่เลือก (KPI + ครัว + แนวโน้ม) | `dashboard.view.read`  | Query: date?, days=7        | `BaseResponseModel<DashboardOverviewResponseModel>`       |
| GET    | `/top-selling`  | เมนูขายดี Top 5 แยกประเภท                | `dashboard.view.read`  | Query: date?, days=30       | `BaseResponseModel<TopSellingResponseModel>`               |
| GET    | `/peak-hours`   | ช่วงเวลาขายดี (จำนวนออเดอร์ต่อชั่วโมง)  | `dashboard.view.read`  | Query: date?                | `BaseResponseModel<PeakHoursResponseModel>`                |
| GET    | `/sales-report` | รายงานยอดขายช่วงเวลา                     | `dashboard.view.read`  | Query: from, to             | `BaseResponseModel<SalesReportResponseModel>`              |

> **Response Models สำคัญ:**
> - `DashboardKpiModel`: TotalSales, OrderCount, GuestCount, AveragePerOrder
> - `KitchenBreakdownModel`: CategoryType (1=อาหาร, 2=เครื่องดื่ม, 3=ของหวาน), CategoryName, ItemCount, Percentage
> - `RevenueTrendModel`: Date, TotalSales
> - `TopSellingItemModel`: MenuId, MenuName, TotalQuantity (Top 5 per CategoryType)
> - `HourlyOrderModel`: Hour (0-23), OrderCount
> - `DailyBreakdownModel`: Date, TotalSales, OrderCount, GuestCount, AveragePerOrder
> - `CategoryBreakdownModel`: CategoryType, CategoryName, TotalSales, Percentage

> **Business Rules:**
> - Overview: เปรียบเทียบ KPI ของวันที่เลือกกับวันก่อนหน้า (calcChange %)
> - Revenue Trend: ย้อนหลัง 7 หรือ 30 วันจากวันที่เลือก
> - Sales Report: from ต้อง <= to, ช่วงเวลาต้องไม่เกิน 365 วัน
> - ใช้ข้อมูลจาก TbOrderBill (Status=Paid), TbOrderItem (ไม่รวม Voided/Cancelled), TbOrder
